package com.guann1n9.leetcode.problems.backtrack;

import com.alibaba.fastjson.JSON;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;
import java.util.stream.Collectors;

/**
 * 给定一个无重复元素的数组 candidates 和一个目标数 target ，找出 candidates 中所有可以使数字和为 target 的组合。
 *
 * candidates 中的数字可以无限制重复被选取。
 *
 * 说明：
 *
 * 所有数字（包括 target）都是正整数。
 * 解集不能包含重复的组合。 
 * 示例 1：
 *
 * 输入：candidates = [2,3,6,7], target = 7,
 * 所求解集为：
 * [
 *   [7],
 *   [2,2,3]
 * ]
 * 示例 2：
 *
 * 输入：candidates = [2,3,5], target = 8,
 * 所求解集为：
 * [
 *   [2,2,2,2],
 *   [2,3,3],
 *   [3,5]
 * ]
 *  
 *
 * 提示：
 *
 * 1 <= candidates.length <= 30
 * 1 <= candidates[i] <= 200
 * candidate 中的每个元素都是独一无二的。
 * 1 <= target <= 500
 */
public class SumTarget {

    static ArrayList<List<Integer>> result = new ArrayList<>();

    public static List<List<Integer>> combinationSum(int[] candidates, int target) {
        Arrays.sort(candidates);//升序排列
        ArrayList<Integer> list = new ArrayList<>();
        backtrack(0,target,candidates,list);
        return result;

    }


    public static void backtrack(int idx,int target,int[] candidates,List<Integer> list){
        if(target == 0){
            result.add(new ArrayList<>(list));
            return;
        }
        for (int i = idx; i < candidates.length; i++) {
            if(target - candidates[i] >= 0){
                list.add(candidates[i]);
                backtrack(i, target-candidates[i], candidates, list);
                list.remove(list.size()-1);
            }else {
                break;
            }
        }
    }


    public static void main(String[] args) {
        int[] a = {2,3,6,7};
        List<List<Integer>> lists = combinationSum(a, 7);
        System.out.println(JSON.toJSONString(lists));

    }


}
